...
def bayes_parameter_opt_lgb(X, y, init_round=15, opt_round=25, n_folds=5, n_estimators=10000,
    ...
    def lgb_eval(num_leaves, feature_fraction, bagging_fraction, max_depth, lambda_l1, lambda_l2, min_split_gain,min_child_weight):
        params = {'application': 'binary',   # regression 回归    binary 二分类   multiclass 多分类
    ...
    # range
    LGB_Bayes = BayesianOptimization(lgb_eval, pbounds={'num_leaves': (25, 50),
...
def bayesOpt(data,feature_category):
    """
    :param data: 训练集 带index 和target
    :param feature_category:  类别特征
    :return:
    """
    y = data[G_TARGET]
    X = data.drop([G_TARGET, G_INDEX], axis=1)
    categorical_feats = feature_category.values.tolist()[0]

    opt_params,append_params = bayes_parameter_opt_lgb(X, y, init_round=5, opt_round=10, n_folds=5, n_estimators=100,
                                         learning_rate=0.05, categorical_feats=categorical_feats)
    ...
    return max_score,max_par

if __name__ == '__main__':

    data = pd.read_csv('process_data/1_train_data.csv')
    feature_category = pd.read_csv('process_data/1_feature_category.csv')

    max_score,max_par=bayesOpt(data,feature_category)
    print("------------------------------------------")
    print(max_score)
    print(max_par)
